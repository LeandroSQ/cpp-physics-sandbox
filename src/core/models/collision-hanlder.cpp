...
std::optional<CollisionInfo> CollisionHandler::getCollisionInfo(const Shape &a, const Shape &b) {
	// Ignore if AABBs are not colliding
	if (a.position.x + a.radius < b.position.x - b.radius || a.position.x - a.radius > b.position.x + b.radius ||
		a.position.y + a.radius < b.position.y - b.radius || a.position.y - a.radius > b.position.y + b.radius) {
		return std::nullopt;
	}
